...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        [childClass setMyName:@"My name is child class"];   //자식클래스인 FirstChildClass의 인스턴스에서
                                                            //MyFirstClass에 구현하였던 인스턴스 메소드를 사용할 수 있다.
        NSLog(@"%@",[childClass myName]);
        
        [FirstChildClass itIsClassMethod];                  //현재 itIsClassMethod는 오버라이드로 되어있다.
        
        [[childClass superclass] itIsClassMethod];          //오버라이드 되지 않은 메소드를 호출하려면
                                                            //superclass의 메소드를 호출한다.
        
        [childClass getMySecret];                           //getMySecret 메소드로 재정의 하였다.
...
/*
 ...
 [FirstChildClass itIsClassMethod];                  //물론 클래스 메소드도 사용이 가능하다.
 */
